...
try:
    ...
    cats_dogs_dir = os.path.join(base_dir, 'cats-v-dogs')
    make_dir(cats_dogs_dir)

    train_dir = os.path.join(cats_dogs_dir, 'train')
    test_dir = os.path.join(cats_dogs_dir, 'test')
    make_dir(train_dir)
    make_dir(test_dir)
...
for fn in uploaded:
    img = image.load_img(os.path.join('dataset/', fn), target_size=(150, 150))
